...
SAM_fit_sol_7a$data$fleetTypes
fit<-runwithout(SAM_fit_sol_7a, fleet = 2)
str(fit)
key <- fit$conf$keyLogFpar[-fit$data$fleetTypes!=0, ] + 1
key[key==0]<-NA
cf<-coef(fit)
cfsd <- attr(cf,"sd")
qt <- cf[ names(cf)=="logFpar" ][key]
rownames(qt) <- attr(fit$data, "fleetNames")[-fit$data$fleetTypes!=0]
colnames(qt)<-fit$conf$minAge:fit$conf$maxAge
sds <- qt #-1.40704 (higher) (met de UK BTS = -1.682 (lower)) If the model estimates a higher q, but the index value remains fixed, then the model must estimate a lower fishable biomass to satisfy the equation:

#To check how the numbers are influenced by leaving out a dat source
ntable(leaveout_fit[[1]])/ntable(SAM_fit_sol_7a)
...
Fmsy_lw         <- 0.125
Fmsy_up         <- 0.183
Fpa             <- 0.183 #(FP.05 (5% risk to Blim with Btrigger))
Flim            <- 0.570
Blim            <- 2235
Btrigger        <- 3129
...
Flast <- fbartable(SAM_fit_sol_7a)[nrow(fbartable(SAM_fit_sol_7a)),1]# from SAM assessment #0.1972142

# mean F last 3 data years
Fsq <- mean(fbartable(SAM_fit_sol_7a)[((nyears-2):(nyears)),1]) #0.1361496

## Catch intermediate year
catch_int_year  <- 545 #The TAC 2026 (545)
...
median_ssb<-median(rowSums(ssb_by_age_sims[,1:3])/rowSums(ssb_by_age_sims)) #0.3529877
taf.png("Contribution_SSB.png", width = 15, height = 15, units = "cm", pointsize = 10, res= 300)
hist(rowSums(ssb_by_age_sims[,1:3])/rowSums(ssb_by_age_sims), main="", xlab = "Proportion of year classes 2025-2027 in the SSB 2028", ylim= c(0,3000))
abline(v = median_ssb, col = 'blue')
text (0.3, 3000, paste0('median = ',round(median_ssb,4)), col = 'blue')
dev.off()

#Extra
hist(ssb_by_age_sims[,1]/rowSums(ssb_by_age_sims), main="", xlab = "Proportion of year classes 2027 in the SSB 2028", ylim= c(0,4000))
median_ssb_age1<-median(ssb_by_age_sims[,1]/rowSums(ssb_by_age_sims)) 
median_ssb_age1 #0.03056485
hist(ssb_by_age_sims[,2]/rowSums(ssb_by_age_sims), main="", xlab = "Proportion of year classes 2026 in the SSB 2028", ylim= c(0,3000))
median_ssb_age2<-median(ssb_by_age_sims[,2]/rowSums(ssb_by_age_sims))
median_ssb_age2 # 0.1219765
hist(ssb_by_age_sims[,3]/rowSums(ssb_by_age_sims), main="", xlab = "Proportion of year classes 2025 in the SSB 2028", ylim= c(0,3000))
median_ssb_age3<-median(ssb_by_age_sims[,3]/rowSums(ssb_by_age_sims)) 
median_ssb_age3 #0.18092
hist(ssb_by_age_sims[,4]/rowSums(ssb_by_age_sims), main="", xlab = "Proportion of year classes 2024 in the SSB 2028", ylim= c(0,3000))
median_ssb_age4<-median(ssb_by_age_sims[,4]/rowSums(ssb_by_age_sims)) 
median_ssb_age4 #0.2039994
hist(ssb_by_age_sims[,5]/rowSums(ssb_by_age_sims), main="", xlab = "Proportion of year classes 2023 in the SSB 2028", ylim= c(0,3000))
median_ssb_age5<-median(ssb_by_age_sims[,5]/rowSums(ssb_by_age_sims)) 
median_ssb_age5 #0.1296552
hist(ssb_by_age_sims[,6]/rowSums(ssb_by_age_sims), main="", xlab = "Proportion of year classes 2022 in the SSB 2028", ylim= c(0,3000))
median_ssb_age6<-median(ssb_by_age_sims[,6]/rowSums(ssb_by_age_sims)) 
median_ssb_age6 #0.07339905
hist(ssb_by_age_sims[,7]/rowSums(ssb_by_age_sims), main="", xlab = "Proportion of year classes 2021 in the SSB 2028", ylim= c(0,3000))
median_ssb_age7<-median(ssb_by_age_sims[,7]/rowSums(ssb_by_age_sims)) #0.04944376
median_ssb_age7 #0.03630176
...
hist(colSums(C_by_age_sims[1:2,])/colSums(C_by_age_sims), main="", xlab = "Proportion of year classes 2025-2026 in the Catch 2027", ylim= c(0,3200))
abline(v = median_catch, col = 'blue')
text (0.055, 3200, paste0('median = ',round(median_catch,4)), col = 'blue')
dev.off()

#Contribution of Recruitment in Catch - Extra
median_catch<-median(C_by_age_sims[3,]/colSums(C_by_age_sims)) #0.2780881
taf.png("Contribution_Catch_R2024.png", width = 15, height = 15, units = "cm", pointsize = 10, res= 300)
hist(C_by_age_sims[3,]/colSums(C_by_age_sims), main="", xlab = "Proportion of year class 2024 in the Catch 2027", ylim= c(0,3200))
abline(v = median_catch, col = 'blue')
text (0.2, 3200, paste0('median = ',round(median_catch,4)), col = 'blue')
dev.off()

#Extra
hist(C_by_age_sims[1,]/colSums(C_by_age_sims), main="", xlab = "Proportion of year classes 2026 in the Catch 2027", ylim= c(0,3200))
median_catch_age1<-median(C_by_age_sims[1,]/colSums(C_by_age_sims)) #0.004260465
median_catch_age1
hist(C_by_age_sims[2,]/colSums(C_by_age_sims), main="", xlab = "Proportion of year classes 2025 in the Catch 2027", ylim= c(0,6000))
median_catch_age2<-median(C_by_age_sims[2,]/colSums(C_by_age_sims)) #0.06641103
median_catch_age2
hist(C_by_age_sims[3,]/colSums(C_by_age_sims), main="", xlab = "Proportion of year classes 2024 in the Catch 2027", ylim= c(0,6000))
median_catch_age3<-median(C_by_age_sims[3,]/colSums(C_by_age_sims)) #0.2780881
median_catch_age3
hist(C_by_age_sims[4,]/colSums(C_by_age_sims), main="", xlab = "Proportion of year classes 2023 in the Catch 2027", ylim= c(0,5000))
median_catch_age4<-median(C_by_age_sims[4,]/colSums(C_by_age_sims)) #0.2261964
median_catch_age4
hist(C_by_age_sims[5,]/colSums(C_by_age_sims), main="", xlab = "Proportion of year classes 2022 in the Catch 2027", ylim= c(0,5000))
median_catch_age5<-median(C_by_age_sims[5,]/colSums(C_by_age_sims)) # 0.1300892
median_catch_age5
hist(C_by_age_sims[6,]/colSums(C_by_age_sims), main="", xlab = "Proportion of year classes 2021 in the Catch 2027", ylim= c(0,3200))
median_catch_age6<-median(C_by_age_sims[6,]/colSums(C_by_age_sims)) #0.0580732
median_catch_age6
